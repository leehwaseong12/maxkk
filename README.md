...
<head>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .modal-lg{max-width:600px;max-height:86vh;overflow-y:auto;}
  .img-thumb-grid{display:flex;gap:10px;flex-wrap:wrap;margin-top:10px;}
  .img-thumb{position:relative;width:84px;height:84px;border-radius:8px;overflow:hidden;border:1px solid var(--border);flex-shrink:0;}
  .img-thumb img{width:100%;height:100%;object-fit:cover;display:block;}
  .img-thumb-del{position:absolute;top:2px;right:2px;width:20px;height:20px;border-radius:50%;background:rgba(0,0,0,.7);color:#fff;border:none;font-size:12px;cursor:pointer;line-height:20px;text-align:center;padding:0;}
  .nd-img-grid{display:grid;gap:10px;margin:14px 0;}
  .nd-img-grid img{width:100%;border-radius:12px;display:block;}

  .target-dept-block{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:12px;margin-bottom:10px;}
  .target-dept-head{display:flex;gap:8px;align-items:center;margin-bottom:8px;}
  .target-dept-head input{flex:1;}
  .target-dept-remove{flex-shrink:0;}

  .read-dept-title{font-size:12px;font-weight:800;color:var(--text3);margin:14px 0 6px;}
  .read-chip-wrap{display:flex;flex-wrap:wrap;gap:8px;}
  .read-chip{padding:7px 12px;border-radius:99px;border:1px solid var(--border);background:var(--surface);font-size:12px;font-weight:700;color:var(--text2);cursor:pointer;transition:all .2s;font-family:'Noto Sans KR',sans-serif;}
  .read-chip:hover:not(:disabled){border-color:var(--blue);color:var(--text);}
  .read-chip.done{background:var(--green-dim);border-color:rgba(63,185,80,.35);color:var(--green);cursor:default;}
  .read-progress{font-size:12px;color:var(--text2);margin-bottom:8px;font-weight:700;}
  .read-progress-track{height:6px;background:var(--surface);border-radius:99px;overflow:hidden;margin-bottom:12px;}
  .read-progress-fill{height:100%;background:linear-gradient(90deg,var(--green),#10b981);border-radius:99px;transition:width .3s;}
  .read-row{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:7px 0;border-bottom:1px solid rgba(255,255,255,.05);font-size:13px;}
  </style>
</head>
<body>
<div id="app">
...
/* 상태: 사진 첨부 / 읽음 확인 대상자 */
let pendingNoticeImages=[], pendingNoticeTargets=[];
let noticeCache={}, adminNoticeCache={};
let currentDetailNoticeId=null, noticeDetailReadsUnsub=null, noticeReadModalUnsub=null;

function mkReadId(dept,name){
  const s=(String(dept||'').trim()+'__'+String(name||'').trim()).replace(/[\/.#$\[\]]/g,'_').replace(/\s+/g,'_');
  return (s||'id_'+Date.now()).slice(0,300);
}

/* 이미지: 리사이즈 후 base64 변환 */
function compressImageFile(file,maxW=1000,quality=0.65){
...
async function handleNoticeImageSelect(evt){
  const files=Array.from(evt.target.files||[]);
  for(const f of files){
    if(pendingNoticeImages.length>=4){ alert('사진은 최대 4장까지 첨부할 수 있습니다.'); break; }
    try{
      const b64=await compressImageFile(f);
      const totalLen=pendingNoticeImages.reduce((s,x)=>s+x.length,0)+b64.length;
      if(totalLen>900000){ alert('사진 용량이 너무 큽니다. 사진 수를 줄이거나 더 작은 사진을 사용해 주세요.'); continue; }
      pendingNoticeImages.push(b64);
    }catch(e){ console.error(e); alert('이미지 처리 중 오류가 발생했습니다: '+e.message); }
  }
  evt.target.value='';
  renderNoticeImagePreview();
}
function renderNoticeImagePreview(){
  const wrap=document.getElementById('nImagePreview'); if(!wrap) return;
  wrap.innerHTML=pendingNoticeImages.map((src,i)=>`<div class="img-thumb"><img src="${src}"/><button type="button" class="img-thumb-del" onclick="removeNoticeImage(${i})">✕</button></div>`).join('');
}
function removeNoticeImage(i){ pendingNoticeImages.splice(i,1); renderNoticeImagePreview(); }

/* 읽음 확인 대상자 설정 모달 */
function openNoticeTargetModal(){
  let modal=document.getElementById('noticeTargetModal');
  if(!modal){
    document.body.insertAdjacentHTML('beforeend',`
      <div class="modal-overlay" id="noticeTargetModal">
        <div class="modal modal-lg">
          <h3>👥 읽음 확인 대상자 설정</h3>
          <p>부서별로 성명을 입력하세요. 등록된 인원은 공지 상세 화면에서 본인 이름을 눌러 읽음을 확인할 수 있습니다.</p>
          <div id="targetDeptList"></div>
          <button type="button" class="sub-btn" style="background:var(--surface);color:var(--text2)" onclick="addTargetDeptRow()">+ 부서 추가</button>
          <div class="modal-actions">
            <button type="button" class="sub-btn btn-admin" style="flex:1" onclick="saveNoticeTargets()">저장</button>
            <button type="button" class="sub-btn" style="background:var(--surface);color:var(--text2);flex:1" onclick="closeNoticeTargetModal()">취소</button>
          </div>
        </div>
      </div>`);
    modal=document.getElementById('noticeTargetModal');
  }
  if(!pendingNoticeTargets.length) pendingNoticeTargets=[{dept:'',names:[]}];
  renderTargetDeptList();
  modal.classList.add('show');
}
function closeNoticeTargetModal(){ const m=document.getElementById('noticeTargetModal'); if(m) m.classList.remove('show'); }
function renderTargetDeptList(){
  const wrap=document.getElementById('targetDeptList'); if(!wrap) return;
  wrap.innerHTML=pendingNoticeTargets.map((g,i)=>`
    <div class="target-dept-block">
      <div class="target-dept-head">
        <input class="inp" placeholder="부서명 (예: 생산1팀)" value="${esc(g.dept)}" oninput="updateTargetDept(${i},this.value)"/>
        <button type="button" class="del-btn target-dept-remove" onclick="removeTargetDept(${i})">삭제</button>
      </div>
      <textarea class="inp" placeholder="성명을 쉼표(,) 또는 줄바꿈으로 구분해 입력 (예: 홍길동, 김철수)" style="min-height:70px">${esc((g.names||[]).join(', '))}</textarea>
    </div>`).join('');
  wrap.querySelectorAll('.target-dept-block textarea').forEach((ta,i)=>{
    ta.addEventListener('input',()=>updateTargetNames(i,ta.value));
  });
}
function addTargetDeptRow(){ pendingNoticeTargets.push({dept:'',names:[]}); renderTargetDeptList(); }
function removeTargetDept(i){ pendingNoticeTargets.splice(i,1); if(!pendingNoticeTargets.length) pendingNoticeTargets=[{dept:'',names:[]}]; renderTargetDeptList(); }
function updateTargetDept(i,v){ if(pendingNoticeTargets[i]) pendingNoticeTargets[i].dept=v; }
function updateTargetNames(i,v){ if(pendingNoticeTargets[i]) pendingNoticeTargets[i].names=v.split(/[,\n]/).map(s=>s.trim()).filter(Boolean); }
function saveNoticeTargets(){
  pendingNoticeTargets=pendingNoticeTargets.filter(g=>g.dept.trim()&&g.names.length);
  updateTargetSummary();
  closeNoticeTargetModal();
}
function updateTargetSummary(){
  const el=document.getElementById('nTargetSummary'); if(!el) return;
  if(!pendingNoticeTargets.length){ el.textContent='설정된 대상자 없음'; return; }
  const total=pendingNoticeTargets.reduce((s,g)=>s+g.names.length,0);
  el.textContent=pendingNoticeTargets.map(g=>`${g.dept}(${g.names.length}명)`).join(', ')+` · 총 ${total}명`;
}

/* 공지 상세 화면 읽음 확인 섹션 */
function renderReadSection(d,readsMap){
  if(!d.targets||!d.targets.length) return '';
  let total=0,done=0;
  const groups=d.targets.map(g=>{
    const chips=(g.names||[]).map(n=>{
      total++;
      const rid=mkReadId(g.dept,n);
      const isDone=!!readsMap[rid];
      if(isDone) done++;
      return `<button type="button" class="read-chip${isDone?' done':''}" data-dept="${esc(g.dept)}" data-name="${esc(n)}"${isDone?' disabled':''}>${isDone?'✅ ':''}${esc(n)}</button>`;
    }).join('');
    return `<div class="read-dept-title">${esc(g.dept)}</div><div class="read-chip-wrap">${chips}</div>`;
  }).join('');
  const pct=total?Math.round(done/total*100):0;
  return `<div class="panel" style="margin-top:16px">
    <div style="font-size:13px;font-weight:800;color:var(--text2);margin-bottom:6px">📖 읽음 확인 — 본인 이름을 눌러주세요</div>
    <div class="read-progress">${done} / ${total}명 확인 (${pct}%)</div>
    <div class="read-progress-track"><div class="read-progress-fill" style="width:${pct}%"></div></div>
    ${groups}
  </div>`;
}
document.addEventListener('click',function(e){
  const btn=e.target.closest('.read-chip');
  if(!btn||btn.disabled) return;
  const dept=btn.getAttribute('data-dept'), name=btn.getAttribute('data-name');
  if(!currentDetailNoticeId) return;
  if(!confirm(`${name}님, 본인이 맞으면 확인을 눌러 읽음 처리합니다.`)) return;
  markNoticeRead(currentDetailNoticeId,dept,name);
});
async function markNoticeRead(noticeId,dept,name){
  try{
    await db.collection('notices').doc(noticeId).collection('reads').doc(mkReadId(dept,name))
      .set({dept,name,readAt:firebase.firestore.FieldValue.serverTimestamp()});
  }catch(e){ alert('읽음 처리 중 오류: '+e.message); }
}
function subscribeNoticeReads(id){
  if(noticeDetailReadsUnsub){ noticeDetailReadsUnsub(); noticeDetailReadsUnsub=null; }
  noticeDetailReadsUnsub=db.collection('notices').doc(id).collection('reads').onSnapshot(snap=>{
    const map={}; snap.docs.forEach(rd=>{ map[rd.id]=rd.data(); });
    const d=noticeCache[id]; if(!d) return;
    const holder=document.getElementById('noticeReadSectionWrap');
    if(holder) holder.innerHTML=renderReadSection(d,map);
  });
}

/* 관리자: 읽음 현황 모달 */
function openNoticeReadModal(id){
  const d=adminNoticeCache[id]; if(!d) return;
  let modal=document.getElementById('noticeReadModal');
  if(!modal){
    document.body.insertAdjacentHTML('beforeend',`
      <div class="modal-overlay" id="noticeReadModal">
        <div class="modal modal-lg">
          <h3>📖 읽음 확인 현황</h3>
          <p id="noticeReadModalSub"></p>
          <div id="noticeReadModalBody"></div>
          <div class="modal-actions">
            <button type="button" class="sub-btn" style="background:var(--surface);color:var(--text2);flex:1" onclick="closeNoticeReadModal()">닫기</button>
          </div>
        </div>
      </div>`);
    modal=document.getElementById('noticeReadModal');
  }
  document.getElementById('noticeReadModalSub').textContent=d.title;
  modal.classList.add('show');
  if(noticeReadModalUnsub){ noticeReadModalUnsub(); noticeReadModalUnsub=null; }
  noticeReadModalUnsub=db.collection('notices').doc(id).collection('reads').onSnapshot(snap=>{
    const map={}; snap.docs.forEach(rd=>{ map[rd.id]=rd.data(); });
    renderNoticeReadModalBody(id,d,map);
  });
}
function closeNoticeReadModal(){
  const m=document.getElementById('noticeReadModal'); if(m) m.classList.remove('show');
  if(noticeReadModalUnsub){ noticeReadModalUnsub(); noticeReadModalUnsub=null; }
}
function renderNoticeReadModalBody(noticeId,d,map){
  const body=document.getElementById('noticeReadModalBody'); if(!body) return;
  let total=0,done=0;
  const html=(d.targets||[]).map(g=>{
    const rows=(g.names||[]).map(n=>{
      ...
      return `<div class="read-row">
        <span style="color:${r?'var(--green)':'var(--text2)'}">${r?'✅':'⬜'} ${esc(n)}</span>
        ${r?`<span style="font-size:11px;color:var(--text3);display:flex;align-items:center;gap:6px">${esc(tsf(r.readAt))}<button type="button" class="del-btn" onclick="resetNoticeRead('${noticeId}','${rid}')">취소</button></span>`:'<span style="font-size:11px;color:var(--text3)">미확인</span>'}
      </div>`;
    }).join('');
    return `<div class="read-dept-title">${esc(g.dept)}</div>${rows}`;
  }).join('');
  const pct=total?Math.round(done/total*100):0;
  body.innerHTML=`<div class="read-progress">${done} / ${total}명 확인 (${pct}%)</div><div class="read-progress-track"><div class="read-progress-fill" style="width:${pct}%"></div></div>`+html;
}
async function resetNoticeRead(noticeId,rid){
  if(!confirm('이 읽음 확인을 취소하시겠습니까?')) return;
  try{ await db.collection('notices').doc(noticeId).collection('reads').doc(rid).delete(); }
...
function renderNoticeList(docs){
  const wrap=document.getElementById('noticeListWrap'); if(!wrap) return;
  docs.forEach(d=>{ noticeCache[d.id]=d; });
  ...
  wrap.innerHTML=`<div class="notice-list">${docs.map(d=>`
    <div class="ni${d.pinned?' pinned':''}">
      ${nBadge(d.category)}
      <div class="ni-body" onclick="showNoticeDetail('${d.id}')">
        <div class="ni-title">${d.pinned?'📌 ':''}${esc(d.title)}</div>
        <div class="ni-summary">${esc(d.summary||'')}</div>
        <div class="ni-meta">${esc(d.dateStr||'')} · ${esc(d.author||'안전관리팀')}${d.images&&d.images.length?' · 🖼️ '+d.images.length:''}</div>
      </div>
...
function showNoticeDetail(id){
  ...
  const imgsHtml=(d.images&&d.images.length)?`<div class="nd-img-grid">${d.images.map(src=>`<img src="${src}"/>`).join('')}</div>`:'';
  document.getElementById('noticeDetailInner').innerHTML=`
    ...
    <div id="noticeReadSectionWrap"></div>`;
  subscribeNoticeReads(id);
...
pageInit['admin-notice']=function(container){
  ...
  pendingNoticeImages=[]; pendingNoticeTargets=[];
  container.innerHTML=`
    <div class="page-header"><button class="back-btn" onclick="goBack()">← 뒤로</button>
    ...
    <div class="admin-form-panel">
      ...
      <div class="form-grid">
        ...
        <div class="form-full">
          <label style="display:block;font-size:12px;font-weight:700;color:var(--text2);margin-bottom:6px">🖼️ 사진 첨부 (최대 4장)</label>
          <input class="inp" type="file" id="nImageInput" accept="image/*" multiple onchange="handleNoticeImageSelect(event)"/>
          <div class="img-thumb-grid" id="nImagePreview"></div>
        </div>
        <div class="form-full" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:4px">
          <button type="button" class="sub-btn" style="background:var(--surface);color:var(--text2);margin-top:0" onclick="openNoticeTargetModal()">👥 읽음 확인 대상자 설정</button>
          <span id="nTargetSummary" style="font-size:12px;color:var(--text3)">설정된 대상자 없음</span>
        </div>
      </div>
...
function subscribeAdminNoticeList(){
  ...
  adminNoticeUnsub=db.collection('notices').orderBy('createdAt','desc')
    .onSnapshot(snap=>{
      ...
      items.forEach(d=>{ adminNoticeCache[d.id]=d; });
      ...
      el.innerHTML=`<div class="notice-list">${items.map(d=>`
        <div class="ni${d.pinned?' pinned':''}">
          ...
          <div class="ni-body"><div class="ni-title">${d.pinned?'📌 ':''}${esc(d.title)}</div>
            <div class="ni-meta">${esc(d.dateStr||'')} · ${esc(d.author||'')}${d.images&&d.images.length?' · 🖼️ '+d.images.length:''}</div></div>
          <div style="display:flex;gap:6px;flex-shrink:0;flex-wrap:wrap">
            ${d.targets&&d.targets.length?`<button class="del-btn" style="color:var(--green);border-color:rgba(63,185,80,.3)" onclick="openNoticeReadModal('${d.id}')">읽음현황</button>`:''}
...
async function submitNotice(){
  ...
  try{
    if(editingNoticeId){
      await db.collection('notices').doc(editingNoticeId).update({title,category:cat,summary,author,content,pinned,images:pendingNoticeImages,targets:pendingNoticeTargets});
      ok.textContent='✅ 공지가 수정되었습니다.';
    } else {
      await db.collection('notices').add({title,category:cat,summary,author,content,pinned,images:pendingNoticeImages,targets:pendingNoticeTargets,createdAt:firebase.firestore.FieldValue.serverTimestamp()});
...
function resetNoticeForm(){
  ...
  pendingNoticeImages=[]; pendingNoticeTargets=[];
  renderNoticeImagePreview(); updateTargetSummary();
...
async function editNotice(id){
  ...
  pendingNoticeImages=Array.isArray(d.images)?d.images.slice():[];
  pendingNoticeTargets=Array.isArray(d.targets)?JSON.parse(JSON.stringify(d.targets)):[];
  renderNoticeImagePreview(); updateTargetSummary();
...
async function submitReport(){
  ...
  try{
    ...
    ok.textContent='✅ 신고가 접수되었습니다.'; ok.style.display='block';
